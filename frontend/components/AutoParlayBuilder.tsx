...
import { Sparkles, Loader2, Trophy, AlertCircle, TrendingDown } from 'lucide-react';
...
export default function AutoParlayBuilder() {
  ...
  const buildParlay = async () => {
    ...
    try {
      const response = await fetch('/api/v1/auto-parlay/build', {
        method: 'POST',
...
• Build a cross-sport parlay (NFL + NBA)"
          ...
          <Button
            ...
            className="w-full bg-gradient-to-r from-blue-600 to-blue-500 hover:from-blue-500 hover:to-blue-400 text-white font-bold py-6 text-lg shadow-lg shadow-blue-600/20"
          >
            {loading ? (
              <>
